...
const wsUri = 'ws://138.197.122.43:8888/kurento'; 
...
const getKurentoClient = async () => {
  ...
  try {
    ...
    console.log('Connected to Kurento Media Server', kurentoClient);
    return kurentoClient;
  } catch (error) {
    console.error('Error connecting to Kurento Media Server:', error);
  }
};
...
io.on('connection', (socket) => {
  ...
  socket.on('joinVideoCall', async ({ user1Id, user2Id }) => {
    ...
    socket.emit('joinedVideoCall', { roomId: roomName });
  });
 ...
 socket.on('offer', async ({ room, offer }) => {
  ...
  try {
    ...
    const sdpAnswer = await webRtcEndpoint.processOffer(offer);
    webRtcEndpoint.gatherCandidates();
    socket.emit('answer', { room, sdpAnswer });
